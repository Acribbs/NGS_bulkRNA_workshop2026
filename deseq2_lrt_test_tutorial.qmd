...
# 1. Restart R session: Session -> Restart R (in RStudio/Positron)
# 2. Reinstall quarto: quarto install tool tinytex (if needed)
# 3. Check quarto: quarto --version
# 4. If issues persist, restart your IDE completely
...
```{r load-data}
# Load metadata
metadata <- read.table("GSE60450_metadata_clean.txt",
                       header = TRUE,
                       sep = "\t",
                       stringsAsFactors = FALSE)

# Load counts
counts <- read.table("GSE60450_counts_ensembl.txt",
                     header = TRUE,
                     sep = "\t",
                     stringsAsFactors = FALSE)

# Load gene annotations
gene_annot <- read.table("GSE60450_gene_annotation.txt",
                         header = TRUE,
                         sep = "\t",
                         stringsAsFactors = FALSE)
